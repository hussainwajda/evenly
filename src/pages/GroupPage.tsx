import { Copy, Handshake, LogOut, MoreVertical, Plus, Shuffle, UserPlus, Users } from 'lucide-react'
...
type Tab = 'expenses' | 'activity' | 'people'
...
export function GroupPage() {
  ...
  async function toggleSimplify() {
    try {
      ...
      await runSync()
      toast(data!.group.simplifyDebts ? 'Showing who owes whom directly' : 'Debts simplified to the fewest payments')
    } catch (e) {
    ...
    <>
      ...
      <div className="space-y-6 px-4 pt-2 lg:max-w-3xl">
        <Panel className="space-y-4">
          ...
          {data.transfers.length ? (
            <ul className="space-y-2">
              {data.transfers.map((t) => {
                ...
                return (
                  <li key={`${t.from}>${t.to}`} className="flex min-h-11 items-center gap-3">
                    <span className={cn('min-w-0 flex-1 text-sm', involvesMe ? 'font-medium' : 'text-muted-foreground')}>
          ...
          <p className="text-xs text-muted-foreground">{data.group.simplifyDebts ? 'Simplified to the fewest payments.' : 'Showing who owes whom directly.'}</p>
        ...
        {tab === 'expenses' ? <ExpenseFeed data={data} /> : tab === 'activity' ? <ActivityList data={data} /> : <PeopleList data={data} onInvite={() => setInviteOpen(true)} />}
      </div>
  ...
  return (
    <ul className="divide-y overflow-hidden rounded-2xl border bg-card">
      {feed.map((item) => {
        ...
        const toMe = s.to === data.meId
        return (
          <li key={`s-${s.id}`} className="flex min-h-16 items-center gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-sm text-muted-foreground">
                ...
                {s.status === 'recorded' ? ' · waiting for confirmation' : s.status === 'disputed' ? ' · not received' : ' · confirmed'}
              </p>
            </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="size-11 shrink-0 rounded-full" aria-label="Payment options">
                  <MoreVertical className="size-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                {toMe && s.status !== 'confirmed' ? (
                  <DropdownMenuItem className="min-h-11" onSelect={() => void setSettlementStatus(db, [s.id], 'confirmed')}>
                    Confirm received
                  </DropdownMenuItem>
                ) : null}
                {toMe && s.status === 'recorded' ? (
                  <DropdownMenuItem className="min-h-11" onSelect={() => void setSettlementStatus(db, [s.id], 'disputed')}>
                    Not received
                  </DropdownMenuItem>
                ) : null}
                <DropdownMenuItem
                  className="min-h-11 text-negative focus:text-negative"
                  onSelect={async () => {
                    await deleteSettlement(db, s.id)
                    toast('Payment deleted', { action: { label: 'Undo', onClick: () => void restoreSettlement(db, s.id) } })
                  }}
                >
                  Delete payment
                </DropdownMenuItem>
...
function describe(item: ActivityItem, actor: string): string {
  ...
  switch (item.action) {
    ...
    case 'expense.updated':
      return `${actor} edited ${title}${amount}`
    case 'expense.deleted':
      return `${actor} deleted ${title}`
    case 'settlement.created':
      return `${actor} recorded a payment${amount}`
    case 'settlement.updated':
      return s.status === 'confirmed' ? `${actor} confirmed a payment${amount}` : s.status === 'disputed' ? `${actor} marked a payment as not received` : `${actor} updated a payment`
    case 'settlement.deleted':
      return `${actor} deleted a payment${amount}`
    default:
...
function ActivityList({ data }: { data: GroupData }) {
  ...
  const changeCount = data.expenses.length + data.settlements.length
      ...
      {items.map((item) => {
        ...
        return (
          <li key={item.id} className="flex items-start gap-3 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm">{describe(item, actor)}</p>
